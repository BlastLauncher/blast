...
transparency, icon serialization, Detail roots, and event routing. Capability
tests cover brokered clipboard writes and reads, denied-write errors, and
unconfigured context errors. Unmeasured surface (shortcuts, object icons,
List-level actions, non-action children) raises structured compatibility
errors.
...
pushes, action groups with titles and submenus, tinted icons, and brokered
capabilities); gap fixtures assert structured `unsupported_api` errors with
non-zero exit. The generated report lives in
