...
The declaration inventory uses the TypeScript checker against the pinned
Raycast declaration and the emitted adapter declaration. Its tests cover
merged namespaces, aliased exports, enum/object members, compiler-metadata
filtering, missing-member detection, and byte-stable output. A clean inventory
does not replace behavior tests or the end-to-end corpus probe. The corpus
probe derives its named-import support set from the emitted declaration and
embeds the inventory/runtime-export result alongside its per-extension outcome
report.
