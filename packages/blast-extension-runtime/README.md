...
`createSceneChannel(session)` carries scene traffic over the runtime session
(ADR 0008). `publish` validates a transaction and sends it as
`scene.transaction`; `onEvent` registers the handler for valid `scene.event`
messages; `handleMessage` routes one received protocol message and fails the
session when a scene event payload is invalid, because application messages
are untrusted until validated. The Raycast compatibility adapter will build
on this channel instead of replacing it.
